...
define(`EXTRAFUNS',`let log1pexp = rf_log1pexp
let qnorm = qnorm5
let pnorm = pnorm5
let dnorm = dnorm4
(* additional functions *)
let poisson_ci x conflevel alternative =
    let alpha = (1.0 -. conflevel) /. 2.0 in
    let pL x alpha = if x=0.0 then 0.0 else qgamma alpha x 1.0 1 0 in
    let pU x alpha = qgamma (1.0-.alpha) (x+.1.0) 1.0 1 0 in
    match alternative with
	    Less -> (0.0, pU x (1.0 -. conflevel))
	  | Greater -> (pL x (1.0 -. conflevel), $1)
	  | TwoSided -> (pL x alpha, pU x alpha);;
let for_loop f init (from, upto) =
    let rec loop(i,y) = if i>upto then y else loop(i+1,f(i,y)) in
    loop(from,init)
let count predicate = for_loop (fun (i, y) -> if predicate(i) then y+1 else y) 0
let poisson_test (x: float) (t: float) (r: float) alternative =
    let m = r*.t in
    match alternative with
	    Less -> ppois x m 1 0
	  ...
	  | TwoSided ->
		...
		let relErr = 1.0 +. 1.0e-7 in
                let d = dpois x m 0 in
		let dstar = d *. relErr in
		let pred i = (dpois (float_of_int i) m 0)<=dstar in
		if x=m then 1.0
		else if x<m then
			let rec loop n = if (dpois (float_of_int n) m 0)>d then loop(n*2) else n in
                        let n = loop(int_of_float(ceil(2.0*.m-.x))) in
                        let y = count pred (int_of_float(ceil m), n) in
			((ppois x m 1 0) +. (ppois (float_of_int(n-y)) m 0 0))
		else (* x>m *)
			let y = count pred (0,int_of_float(floor(m))) in
			((ppois (float_of_int(y-1)) m 1 0) +. (ppois (x-.1.0) m 0 0))')
divert(`1')dnl
